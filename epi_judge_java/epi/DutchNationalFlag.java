...
public class DutchNationalFlag {
    ...
    public static void dutchFlagPartition(int pivotIndex, List<Color> A) {
        ...
        //put everything smaller than us before us
//        System.out.println("\nBefore Arr: ");
//        A.forEach(c -> System.out.print(c.ordinal() + " "));
//        System.out.println("\nPivot index: " + pivotIndex + ", pivot value: " + A.get(pivotIndex).ordinal());
        Collections.swap(A, 0, pivotIndex);
        pivotIndex = 0;

        for (int index = 1; index < A.size(); index++) {
            if (A.get(index).ordinal() < A.get(pivotIndex).ordinal()) {
                Collections.swap(A, index, pivotIndex);
                pivotIndex++;
                Collections.swap(A, pivotIndex, index);
            }
        }

        //put everything equal to us together with us
        for (int index = pivotIndex + 1; index < A.size(); index++) {
            if (A.get(index).ordinal() == A.get(pivotIndex).ordinal()) {
                Collections.swap(A, index, pivotIndex);
                pivotIndex++;
                Collections.swap(A, pivotIndex, index);
